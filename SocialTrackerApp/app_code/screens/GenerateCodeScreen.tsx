...
import { createUser, hasUser } from "@/db/init";
...
const GenerateCodeScreen = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      let userCreated = await createUser(code)
      if (userCreated) {
        await updateDBData()
        router.replace("/app")
      }
  ...
  return (
    <View style={{ backgroundColor: "white", flex: 1, justifyContent: "center", alignItems: "center", padding: 20 }}>
      <View style={{ alignItems: "center", marginBottom: 20 }}>
        ...
        <Text style={{ fontWeight: 700, fontSize: 14, textAlign: "center", marginTop: 5 }}>Create an account</Text>
        <Text style={{ fontSize: 14, textAlign: "center", marginTop: 0 }}>
          Enter your six digit code to sign up for this app
        </Text>
      </View>
      <View style={{width: "100%",  alignItems: "center"}}>
        <TextInput value={code} placeholder="six digit code" onChangeText={setCode} style={{ borderWidth: 1, padding: 10, width: "80%", marginBottom: 20, borderRadius: 6 }} />
        <Button mode="contained" style={{width: "80%", borderRadius: 8, backgroundColor: "black"}} onPress={async () => await handleGenerate()}>Continues</Button>
